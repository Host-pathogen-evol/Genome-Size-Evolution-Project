...
def trates(Gen,MU): #MU=[Np,m1,m2,m3,m4,m5,m6,m7,m8?,Lth,beta1,beta2]
    ...
    for j in Gen.keys():
        ...
        if Gen[j][0]=='eff':
            ...
            #Duplication
            if L<Lth:
                t3=m5*(Lth-L)*math.exp(-b2*Gen[j][1])
            else:
                t3=0.0
        ...
        if Gen[j][0]=='te':
            ...
            if L<Lth:
                t2=m4*(Lth-L)*math.exp(-b1*Gen[j][1]) #rep
            else:
                t2=0.0

            if ntes>0.0:
              t3=(m7/ntes)*((Gen[j][1]**5)/((Gen[j][1]**5)+((9e3)**5)))
            else:
              t3=0.0 #m7*((Gen[j][1]**5)/((Gen[j][1]**5)+((9e3)**5)))
