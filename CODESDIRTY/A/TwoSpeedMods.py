...
def RefHILL(l,lo,n):
	iyge=0.0
	iyge=pow(lo,n)/(pow(l,n)+pow(lo,n))
	return iyge
...
def INIASSEMBLE(gpo,Ho,HP,rk,prto):
	for i in gpo.keys():
		#print i
		#print gp[i]
		#print gp[i][1]
		...
		for j in Ho:
			...
			if(HP[inx][j-1]==1.0):
				sq=rk.uniform_pos()
			eq=Hill(sq,prto[0],5,3.0)*(RefHILL(gpo[i][2],prto[1],2))
			#smu[].append(sq)
			smu[j]=sq
			#emu.append(eq)
			emu[j]=eq
...
def TRANSFORMATIONS(gn,rn,Gn,Hg,rk,prto):
	#rn 0 ei->ei'
	#rn 1 ei->ei+ei
	#rn 2 ei->eo
	#print Gn
				...
				for sk in sijn.keys():
					...
					for kn in Hg:
						if sk==kn:
							...
							break
					if wqflg==1.0:
						wk=Hill(sijn[sk],prto[0],5,3.0)*(RefHILL(nlen,prto[1],2))
					if wqflg==0.0:
						wk=0.0
	...
	for i in Gnj.keys():
		...
		for jm in smu.keys():
			...
			if((HP[ink][jm-1]!=0.0) and (pthflg==1.0)):
				qj=Hill(smu[jm],prto[0],5,3.0)*(RefHILL(Gnj[i][2],prto[1],2))
			emu[jm]=qj
